...
#Define a function for estimating the parameter lambda
f = function(lambdah, u){
  if (lambdah == 0){
    value = sum(u)-t
  }
  else {
    value = sum(log(1+(exp(lambdah)-1)*u))/lambdah-t
  }
  return(value)
}
...
#Weights W_t(u)
w = function(uu, lambdahat){
    pri = sapply(lambdahat,prior)
    deriv = (exp(lambdahat)/lambdahat)*sum(u/(1+(exp(lambdahat)-1)*u))-t/lambdahat
    value = abs(pri/deriv)
  return(value)
}
...
#First 10 elements are for the burnin and 11th element is chosen as the sample V
xmetrop = unlist(xmetrop[m])

#Plug the generated sample V and the estimated lambda into the function eta
eta = function(u, lambdah){
  if (lambdah == 0){
    value = u
  }
  else{
    value = log(1+(exp(lambdah)-1)*u)/lambdah
  }
  return(value)
}

#Every iteration, add n sample points to vector xmetrop2
xmetrop2 = append(xmetrop2,eta(xmetrop, lambdahat[m]))
